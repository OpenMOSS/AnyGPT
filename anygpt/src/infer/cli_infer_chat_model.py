...
import os
os.environ["http_proxy"]="http://zhanjun.p:Zj20220901.@10.1.8.50:33128/"
os.environ["https_proxy"]="http://zhanjun.p:Zj20220901.@10.1.8.50:33128/"
...
parser.add_argument("--model_name", type=str, default="AnyGPT")    
parser.add_argument("--model-name-or-path", type=str, default="/mnt/petrelfs/zhanjun.p/mllm/output_models/visual_inter_speech_golden_fs/checkpoint-30000")
parser.add_argument("--lora-weights", type=str, default=None)
parser.add_argument("--image-tokenizer-path", type=str, default="/mnt/petrelfs/zhanjun.p/mllm/models/seed-tokenizer-2/seed_quantizer.pt")
parser.add_argument("--speech-tokenizer-path", type=str, default="/mnt/petrelfs/zhanjun.p/mllm/models/speechtokenizer/ckpt.dev")
parser.add_argument("--speech-tokenizer-config", type=str, default="/mnt/petrelfs/zhanjun.p/mllm/models/speechtokenizer/config.json")
parser.add_argument("--inter-testset", type=bool, default=True)
parser.add_argument("--output-dir", type=str, default="/mnt/petrelfs/zhanjun.p/mllm/infer_output/test")
args = parser.parse_args()
os.makedirs(args.output_dir, exist_ok=True)
conv_chatbot=args.model_name

print("chatbot name:", conv_chatbot)
if conv_chatbot == "AnyGPT":
    from m_utils.prompter import *
else:
    from m_utils.prompter_mmgpt import *

conversation = get_conv_template(conv_chatbot)


class MMGPTInference:
    def __init__(
        self, 
        ...
        speech_tokenizer_path:str="/mnt/petrelfs/zhanjun.p/mllm/models/speechtokenizer/ckpt.dev",
        speech_tokenizer_config:str="/mnt/petrelfs/zhanjun.p/mllm/models/speechtokenizer/config.json",
        soundstorm_path:str="/mnt/petrelfs/zhanjun.p/mllm/models/soundstorm/mls_1.pt"
    ):
    ...
    def eval_asr(self, metadata_file_path):
        # 确保输出目录存在
        ...
        for line in tqdm(lines):
            ...
            speech_file_path = entry["speech"]
            ground_truth_text = entry.get("text", "")  # 假设已经有了正确的文本
            recognized_text = self.response('speech', 'text', speech_file_path)
            # 更新原始数据
            entry['asr_text'] = recognized_text
            entry['wer'] = wer(ground_truth_text.lower(), recognized_text.lower())  # 计算WER
            total_wer += entry['wer']
            results.append(entry)

        # 计算平均WER
        avg_wer = total_wer / len(lines) if lines else 0

        # 以当前时间命名json文件
        current_time = datetime.now().strftime('%Y%m%d%H%M%S')
        output_file_path = os.path.join(self.output_dir, f'asr_results_{current_time}.json')
        
        # 将结果写入json文件
        with open(output_file_path, 'w', encoding='utf-8') as json_file:
            json.dump({"results": results, "average_wer": avg_wer}, json_file, ensure_ascii=False, indent=4)

        print(f"ASR评估完成，平均WER为：{avg_wer:.2f}，结果已保存到 {output_file_path}")
    ...
    def forward(
        ...
        # recieve instruction, image
        # interleaved|Can you translate the emotion in this picture into music?|music||data/images/testset/mmichat_sample/ningjing.png
        # interleaved|Can you show the content of music as an image?|image||||data/mmichat_sample/Can you give me a similar style of music.wav
    ...
    def interact(self):
        ...
        prompt = str(input(f"Please talk with {chatbot_name}:\n"))
        while prompt != "quit":
            try:
                self.forward(prompt)
                ...
                traceback.print_exc()
                print(e)
            prompt = str(input(f"Please input prompts for {chatbot_name}:\n"))
            
            
if __name__=='__main__':
    infer = MMGPTInference(
    ...
    # audio_pretrain_4n_2ga/checkpoint-11000